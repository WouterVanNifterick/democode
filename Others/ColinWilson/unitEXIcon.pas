(*======================================================================*
 ...
 |                            fixed.                                    |
 | 1.1.20.1 12/04/2008  CPWW  Tiburon version                           |
 *----------------------------------------------------------------------*)
 ...
 *----------------------------------------------------------------------------*)
procedure MaskBitmapBits (bits : PByte; pixelFormat : TPixelFormat; mask : PByte; width, height : DWORD; palette : HPalette);
var
                ...
                4 :
                  begin
                    ...
                    maskColorByte := maskColor shl (4 - bitbp)
                                             ...
                                             // Apply the mask
              bitp^ := byte ((byte (bitp^) and (not maskByte)) or maskColorByte);
            end;
...
begin
  ...
  try
    ...
    try
      ...
      info := PBitmapInfo (PByte (image.FMemoryImage.Memory) + offset);
      colorBits := PByte (info) + infoHeaderSize;
    ...
    begin
      ...
      try
        ...
        begin
          ...
          with PRGBQUAD (PByte (monoInfo) + sizeof (TBitmapInfoHeader) + sizeof (RGBQUAD))^ do
          begin
            rgbRed := $ff;
            rgbGreen := $ff;
            rgbBlue := $ff;
          ...
          bits := PByte (info) + bitsOffset + bitsSize;
          ...
          with rect do
            StretchDIBits (ACanvas.Handle, left, top, right - left, bottom - top, 0, 0, hdr^.biWidth, hdr^.biHeight, PByte (info) + bitsOffset, info^, DIB_RGB_COLORS, SRCCOPY);
      finally
...
begin
  ...
  try
    ...
    ZeroMemory (@dirEntry, SizeOf (dirEntry));
...
begin
  if Assigned (FMemoryImage) then
    if FIsIcon then
      ...
      result := PBitmapInfo (PByte (FMemoryImage.Memory) + sizeof (DWORD))
  else
    result := Nil
end;
